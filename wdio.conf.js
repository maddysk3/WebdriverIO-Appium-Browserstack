...
exports.config = {
    ...
    onPrepare: async (config, capabilities) => {
        ...
        data.append('file', fs.createReadStream(envVar.apkPath));

        var config = {
            method: 'post',
            url: `https://${envVar.user}:${envVar.key}@api-cloud.browserstack.com/app-automate/upload`,
            headers: {
                ...data.getHeaders()
            },
        ...
        capabilities[0].app = await axios(config)
            .then((response) => {
                appHashId = response.data.app_url;
                global.appHashId = appHashId;
